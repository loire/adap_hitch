...
//	 Trying to parse:
	...
	try
	{
		cfg.lookup("chrom.adaptative_mutation_number");
		cfg.lookup("chrom.adap_s");
		cfg.lookup("chrom.adap_h");
		cfg.lookup("chrom.adaptative_recombination_rate");
		cfg.lookup("chrom.neutral_recombination_rate");
	...
	double h = cfg.lookup("chrom.adap_h");
	double a_r = cfg.lookup("chrom.adaptative_recombination_rate");
	...
	Setting &s_num_vals = cfg.lookup("chrom.adaptative_mutation_number");
	Setting &rn_vals = cfg.lookup("chrom.neutral_recombination_rate");
	...
	for (int m_i=0 ; m_i < m_vals.getLength() ; m_i++)
	{
		m = m_vals[m_i];
		for (int s_i=0; s_i < s_vals.getLength(); s_i++)
		{
			s = s_vals[s_i];
			cout << "m: " << m << " s: " << s << " 2*m/s: " << 2*m/s << endl;
...
//				double m = m_vals[0];		// Migration rate
				int s_num = s_num_vals[0];
//				double s = s_vals[0];
				double n_r = rn_vals[0];
				double U= U_vals[0];
				...
				cout << "number of adaptative mutation: " << s_num << endl;
				cout << "selective values of adaptative mutation: " << s << endl;
				cout << "dominance: " << h << endl;
				cout << "recombination rate between adaptative:" << a_r << endl;
				cout << "recombination rate with neutral locus: " << n_r << endl;
